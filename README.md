...
> 版本：`0.5.2`｜語言：Python 3.14｜套件管理：uv｜AI：n8n AI Agent 微服務｜儲存：MongoDB Atlas｜部署：Docker Compose
...
2. 完成後回覆**緊湊型 Embed**（區塊之間不空行）：店名（點擊即導航）→ **⭐ 評分／評論總數／💰 價位**（最可靠的彙總信號擺第一行）→ 👍 好評／👎 負評條列 → ✅／❌ 結構化事實 → **樣本大小提醒** → 一行小字來源 → 頁尾歸屬。刻意不放地址（貼連結的人點標題就能導航）與一句話總評（資訊量低於底下條列）。
