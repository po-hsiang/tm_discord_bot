...
> 正文最後更新：2026-08-22。功能與檔案結構的細節以 `README.md` 為準，本文件記錄 README 沒有的：協作共識、跨服務架構、環境雷點、待辦。
...
- 功能：混合模式 AI 對話（`!` 指令優先，其餘訊息含圖片/貼圖直達 AI）、YouTube 影片快速摘要（專屬頻道貼連結即觸發）、`!吃`/`!聽`/`!查歌單`、每日 07:30 早安（含全台天氣播報與節日/補假彩蛋）、每晚 19:30 台灣熱門話題、每週五 22:00 遊戲特惠情報。
...
| 遊戲情報要帶哪些 Steam 資料（評價、發行日、線上人數…） | n8n 的 `game_deals` 工具（主人的 n8n Agent 維護）；bot 端只在 `prompts.py` 規定「怎麼講」 |
...
32. **（2026-08-17）Maps 摘要版型定案為緊湊型 ＋ `facts` 上契約**（主人看過實際截圖後點名調整）：
    - **移除**：一句話總評、區塊間空行、地址欄位、來源欄位（整個欄位五行）。**保留但壓成一行小字**的是來源連結——那是 Google 對 grounded 內容的硬性要求（必須標示 Maps 來源且一次互動內可見），不能整個拿掉，故改用「地點／評論 N」短標籤 ＋ 原樣 uri。`ui/maps.py` 有 `TestRemovedSections` 鎖住這些移除，回歸會被抓到。
    - **新增 `facts`（向後相容，只加不改）**：`{"yes": [...], "no": [...], "price_level"}`。`price_level` 併進第一行（⭐ 評分　／　評論數　／　💰 價位），`yes`／`no` 各壓成一行 ✅／❌。**鐵則：沒被列到 ＝ Google 沒登錄，不是「沒有」**（Places API 對不知道的屬性是整個欄位不回傳），絕不可從缺漏推論出「沒有 XX」——`ui/maps.py` 模組 docstring 與 `TestFactsLines` 都寫死了這條。
    - **`place.address` 與 `review.verdict` 仍留在契約裡**（n8n 照舊回傳），只是 bot 端不呈現。日後想加回來不必動 n8n。
    - **實測驗證**：晶華（14,217 則評論、4 則評論來源）→ 樣本提醒正確**不出現**；Klatch（649 則、1 則來源）→ 提醒出現且負評區塊顯示誠實文字；洋朵 → 又一次 `NO_REVIEW_DATA`（累計約 1/4 成功率）。測試 189 → 199。
    - **待主人決定的一項**：晶華的 ✅ 行有 11 個標籤（內用／外帶／三種無障礙／兩種停車／三種付款…），在手機上會佔三行，與「緊湊」的訴求有點衝突。n8n 端上限是 18 且排序即重要性，所以 bot 端要裁很安全；尚未裁，等主人指示。

33. **（2026-08-18）Maps 摘要：意圖分流、空行回復、facts 上限 8**：
    - **意圖分流**（`services/maps.py` 的 `is_bare_link()`）：訊息扣掉連結後只剩空白與標點 → 走**直連** webhook 回卡片；旁邊有任何字 → 走 **AI Agent**（n8n 端已把同一個工作流掛成 Agent 的 `maps_review` 工具）。**規則刻意是確定性的**——不去猜那句話是不是問句，因為猜錯會讓使用者問了問題卻收到卡片、而且無法解釋為什麼。現在的規則一句話講完：「只貼連結給卡片，想聊就多寫一句」。判斷式放服務層當純函式，測試不必碰 discord。
    - **為什麼裸連結要直連而不是全交給 Agent**：① 直連才拿得到 `place/review/facts/sources` 結構化欄位，才組得出卡片——走 Agent 回來是散文，評分、facts 與**來源連結**會被揉掉，而來源是 Google 的硬性要求，不該押在模型的排版習慣上；② 確定性（Agent 有工具不代表每次都會用）；③ 錯誤碼有型別（`NO_REVIEW_DATA` 才能回對應文案）；④ 少一輪 LLM，快又省。
    - **`.env` 維持內網位址**（`host.docker.internal:5678`），不要改用 ngrok：容器→宿主是一跳，繞公網多兩跳且多一個故障點（隧道掛掉／額度用完／攔頁），也沒必要把這條 webhook 曝在公網。ngrok 留給真正的外部呼叫者。
    - **版面**：區塊之間改回空一行（主人偏好）；✅／❌ 併成同一個區塊（它們是一對，不該被空行拆開）；facts 每行上限 8 個標籤，超過補 `…`（**不做無聲截斷**；n8n 端排序即重要性，裁掉的是尾巴）。
    - 測試 199 → 210。

34. **（2026-08-18）Maps 卡片再簡化：拿掉 ✅／❌，來源壓到 `🔗`**（主人要求「可讀性提升到極限」）：
    - **`facts.yes`／`facts.no` 不再呈現**（前一天才加上、隔天就拿掉——晶華有 11 個標籤會佔到三行，把真正要看的評分與評論擠下去）。`facts.price_level` 保留、併在第一行。**契約沒動**，n8n 照舊回傳三個欄位，日後想加回來只改 `ui/maps.py`；鐵則（沒被列到＝Google 沒登錄，不可推論成「沒有」）仍寫在模組 docstring 裡。
...
36. **（2026-08-22）舊指令 `!抽`／`!心結`／`!問`(`!gpt`) 全數移除（v0.8.0）**。主人的判斷：前兩者「回覆固定字串的不好玩」，`!問` 早已被混合模式取代（指定頻道直接說話即可）。保留的是**功能型**指令：`!吃`（動態分類）、`!聽`／`!查歌單`。
    - 刪除 `cogs/draw.py`、`cogs/misc.py`、`services/draw.py`，並自 `bot.py` 的 `EXTENSIONS` 移除兩個 Cog（6 → 4 個）。測試 148 → 147。
    - **`!問` 移除後不會有殘留錯誤**：路由找不到指令時會轉給 `Eat.try_meal()`，比對不到分類就回 `False` 靜默結束——已確認 `!問 你好` 不會有任何回覆，也不會噴錯。
    - **測試裡拿 `!抽` 當「泛用指令範例」的三處已改成 `!聽`**（`test_routing.py`、`test_bot.py`）。那些測試驗的是路由與補空格，不是抽籤本身，換一個仍存在的指令就好。
    - ⚠️ **這件事讓 2026-08-22 那份 DB 功能盤點的第 2 順位失效**：原本建議「抽籤紀錄 ＋ 彩虹籤名人堂 ＋ 週報」，理由是「資料已在產生、只差存下來、情緒濃度最高」。`!抽` 拿掉後這條沒有資料來源了。若日後想要類似的「稀有事件名人堂」，得先有一個會產生稀有結果的新玩法。
